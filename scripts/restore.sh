...
DROP_TABLES_STRING=$(printf "DROP TABLE IF EXISTS %s;" "${TABLES[@]}")

echo "Dropping existing tables..."
docker exec -i $DB_CONTAINER_NAME mysql -u root -p$DB_PASSWORD -e "USE $DB_NAME; $DROP_TABLES_STRING"

echo "Restoring selected tables from dump..."
docker exec -i $DB_CONTAINER_NAME mysql -u root -p$DB_PASSWORD $DB_NAME < "$DUMP_FILE_PATH"
